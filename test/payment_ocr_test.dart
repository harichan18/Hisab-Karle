...
void main() {
  ...
  group('Payment OCR Tests - Real Regression Screenshots', () {
    ...
    test(
      'TEST 1: ₹200.00 -> Amount = 200.0, Receiver = CHANDAN D KUSHWAHA',
      () {
        const rawText = '''
₹200.00
Paid to
...
''';
    ...
    test(
      'TEST 2: ₹98,000.00 -> Amount = 98000.0, Receiver = BINAY KUSHWAHA',
      () {
        const rawText = '''
₹98,000.00
Paid to
...
''';
    ...
    test(
      'TEST 2 (Dropped symbol): 98,000.00 -> Amount = 98000.0, Receiver = BINAY KUSHWAHA',
      () {
        const rawText = '''
98,000.00
Paid to
...
''';
    ...
    test(
      'TEST 3: Paid ₹1 -> Amount = 1.0, Receiver = DIVYANSHU NAGO THAKARE',
      () {
        const rawText = '''
Paid ₹1
...
''';
    ...
    test(
      'TEST 4: PhonePe Received from Divyanshu ₹12 -> Amount = 12.0, Receiver = DIVYANSHU NAGO THAKARE, NOT 202',
      () {
        const rawText = '''
...
''';
    ...
    test(
      'TEST 5: Date line "09:16 pm on 11 Sept 2026" is never parsed as amount 202',
      () {
        const rawText = '''
Transaction Successful
09:16 pm on 11 Sept 2026
...
''';
    ...
    test(
      'TEST 6: PhonePe with dropped currency symbol (standalone "12") -> Amount = 12.0',
      () {
        const rawText = '''
Transaction Successful
09:16 pm on 11 Sept 2026
...
''';
    ...
    test(
      'TEST 7: PhonePe with embedded name and amount "Divyanshu 12" -> Amount = 12.0',
      () {
        const rawText = '''
Transaction Successful
09:16 pm on 11 Sept 2026
...
''';
    ...
    test(
      'TEST 8: Masked account 5649 in XXXXXX5649@sic is rejected, amount 12 is extracted',
      () {
        const rawText = '''
Transaction Successful
09:16 pm on 11 Sept 2026
...
''';
    ...
    test(
      'TEST 10: Navi UPI ₹149 -> Amount = 149.0, NOT 1.0 from B M MOBILE 1',
      () {
        const rawText = '''
Paid securely on
navi UPI
...
''';
    ...
    test(
      'TEST 11: PhonePe amount 12 extracted when only on Credited to line with masked account',
      () {
        const rawText = '''
Transaction Successful
09:16 pm on 11 Sept 2026
...
''';
    ...
    test(
      'TEST 12: PhonePe dropped rupee symbols across both lines -> Amount = 12.0',
      () {
        const rawText = '''
Transaction Successful
09:16 pm on 11 Sept 2026
...
''';
  ...
  group('Indian Currency Formats and AmountParser Tests', () {
    test(
      'AmountParser handles Indian comma formatting and diverse notations',
      () {
  ...
  group('Receiver Matcher & Confidence-Based Friend Matching Tests', () {
    ...
    test(
      'High confidence match: First + Last name match (DIVYANSHU NAGO THAKARE -> Divyanshu Thakare)',
      () {
        ...
        final divyanshuMatch = matches.firstWhere(
          (m) => m.friend.name == 'Divyanshu',
        );
    ...
    test(
      'No matching friend found returns empty candidate list without creating any records',
      () {
        final matches = ReceiverMatcher.matchReceiver(
  ...
  group('Integer Paise Split Calculation Tests', () {
    test('₹500 / 3 becomes 166.67, 166.67, 166.66 summing exactly to ₹500', () {
      ...
      final sumPaise = shares.fold<int>(
        0,
        (sum, s) => sum + AmountParser.toPaise(s.amount)!,
      );
